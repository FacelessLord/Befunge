...
def load_instructions(instructions: dict):
    ...
    # if
    instructions['_'] = lambda c: c.set_direction(Right) if c.stack.pop() == 0 else c.set_direction(Left)
    instructions['|'] = lambda c: c.set_direction(Down) if c.stack.pop() == 0 else c.set_direction(Up)
